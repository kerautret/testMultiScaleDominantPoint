...
double iseContourCircle(const vector<Point>& contour, Point p1, Point p2, Point center, double radius)
{
    double ise = 0, d = 0;
    int indexP1 = findElement(contour,p1);
    int indexP2 = findElement(contour,p2);
...
double iseContourCircle(const vector<Point>& contour, int indexP1, int indexP2, Point center, double radius)
{
    double ise = 0, d = 0;
    for(int it = indexP1+1; it < indexP2; it++)
    {
...
double lmaxContourCircle(const vector<Point>& contour, Point p1, Point p2, Point center, double radius)
{
    double lmax = 0, l = 0;
    int indexP1 = findElement(contour,p1);
    int indexP2 = findElement(contour,p2);
...
double lmaxContourCircle(const vector<Point>& contour, int indexP1, int indexP2, Point center, double radius)
{
    double lmax = 0, l = 0;
    for(int it = indexP1+1; it < indexP2; it++)
    {
...
double error_ISE(const vector<Point>& contour, const vector<Point>& DP, const vector<int>& indexDP, bool isClosed)
{
    ...
    if(isClosed)//consider the last seg DP.front() and DP.back()
    {
        ...
        int indexEnd = contour.size();
        //cout<<"it = "<<it<<" : indexStart = "<<indexStart<<", indexEnd = "<<indexEnd<<endl;
        if(indexStart < 0 || indexEnd < 0)
            cout<<"Pb in error_ISE index of DP"<<endl;
        for(int index = indexStart+1; index < indexEnd; index++)
        {
            d = distancePointSegment(contour.at(index),DP.back(), DP.front());
            //cout<<"index "<<index<<" : distance "<<d<<endl;
            ise += d*d;
        }
    }
